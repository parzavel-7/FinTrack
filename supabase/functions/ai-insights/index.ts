...
serve(async (req) => {
  ...
  try {
    ...
    ${Object.entries(categorySpending).map(([cat, amt]) => `- ${cat}: $${amt.toLocaleString()}`).join("\n")}
    
    Goals Progress:
    ${goalsProgress.length > 0 
      ? goalsProgress.map((g: any) => `- ${g.name}: ${g.progress}% complete, $${g.remaining.toLocaleString()} remaining`).join("\n")
      : "No goals set"
    }
